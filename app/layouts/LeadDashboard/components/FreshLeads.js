...
export default class FreshLeads extends Component {
  static navigationOptions = props => ({
    title: props.navigation.getParam(
      'New ',
      `New (${props.screenProps.count && props.screenProps.count.length > 0 ?
        props.screenProps.count[0].new_leads : 0})`
    ),
  ...
  static getDerivedStateFromProps = (nextProps, prevState) => {
    if (nextProps.screenProps.leads
       && nextProps.screenProps.executives
       && nextProps.screenProps.leads.length > 0
       && nextProps.screenProps.executives.length > 0
    ) {
      return {
        data: nextProps.screenProps.leads,
        executiveData: nextProps.screenProps.executives,
  ...
  onDropDownChange = () => {
    ...
    storage.getItem('currentUser')
      .then(value => {
        ...
        dealer = dealerId;
        return this.props.getLeads('new');
      }).then(() => {
        this.props.getExecutives(dealer).then(() => {
          const filterObj = {
