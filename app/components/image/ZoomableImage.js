...
class ZoomableImage extends Component {
  ...
  componentWillMount() {
    /**
      evt, gestureState can be called from callback
    */
  ...
  processTouch(x, y) {
    if (!this.state.isMoving) {
      ...
      this.setState({
        left: left > 0 ? 0 :
          maxOffset(left, this.state.width, this.props.imageWidth * this.state.zoom),
        top: top > 0 ? 0 :
          maxOffset(top, this.state.height, this.props.imageHeight * this.state.zoom),
      });
    }
  }
